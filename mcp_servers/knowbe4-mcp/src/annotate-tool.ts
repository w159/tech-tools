...
const CLASS_OVERRIDES: Record<string, ToolClass> = {
  knowbe4_groups_members: "read", // GET /groups/{id}/members
  knowbe4_phishing_campaign_tests: "read", // GET /phishing/campaigns/{id}/security_tests
  knowbe4_phishing_security_test_recipients: "read", // GET /phishing/security_tests/{id}/recipients
  knowbe4_phishing_security_test_recipient: "read", // GET /phishing/security_tests/{id}/recipients/{rid}
  knowbe4_reporting_risk_overview: "read", // GET /account risk summary
  // Exposed only when LAZY_LOADING=true, so a default tools/list never sees them.
  knowbe4_router: "read", // suggests tool names for an intent; calls no API
  knowbe4_execute_tool: "destructive", // proxies ANY knowbe4 tool, including the mutating ones
};
...
export function effectClassFor(name: string, description?: string): ToolClass {
  ...
  console.error(
    `[knowbe4-mcp] tool ${name} matches no effect-class pattern; annotating it as ` +
  ...
  return tools.map((t) => {
    if (t.annotations?.readOnlyHint !== undefined) {
      ...
      if (t.annotations.readOnlyHint === true && MUTATING_DESCRIPTION_MARKER.test(t.description ?? "")) {
        console.error(
          `[knowbe4-mcp] tool ${t.name} is annotated readOnlyHint:true but its own ` +
