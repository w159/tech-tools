...
// because there is no shared workspace import root. On copy, replace the
// "vendor-mcp" tag in the stderr messages below with the server directory name.
//
// Usage:
...
export function effectClassFor(name: string, description?: string): ToolClass {
  ...
  console.error(
    `[vendor-mcp] tool ${name} matches no effect-class pattern; annotating it as ` +
  ...
  return tools.map((t) => {
    if (t.annotations?.readOnlyHint !== undefined) {
      ...
      if (t.annotations.readOnlyHint === true && MUTATING_DESCRIPTION_MARKER.test(t.description ?? "")) {
        console.error(
          `[vendor-mcp] tool ${t.name} is annotated readOnlyHint:true but its own ` +
