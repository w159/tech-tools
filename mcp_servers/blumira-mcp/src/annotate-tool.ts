...
const CLASS_OVERRIDES: Record<string, ToolClass> = {
  // Reached only after blumira_navigate selects a domain, so a bare tools/list
  // probe never sees them; both are GETs of a finding's underlying log rows.
  blumira_findings_evidence: "read", // GET /findings/{id}/evidence
  blumira_msp_findings_evidence: "read", // GET /accounts/{id}/findings/{id}/evidence
};
...
export function effectClassFor(name: string, description?: string): ToolClass {
  ...
  console.error(
    `[blumira-mcp] tool ${name} matches no effect-class pattern; annotating it as ` +
  ...
  return tools.map((t) => {
    if (t.annotations?.readOnlyHint !== undefined) {
      ...
      if (t.annotations.readOnlyHint === true && MUTATING_DESCRIPTION_MARKER.test(t.description ?? "")) {
        console.error(
          `[blumira-mcp] tool ${t.name} is annotated readOnlyHint:true but its own ` +
