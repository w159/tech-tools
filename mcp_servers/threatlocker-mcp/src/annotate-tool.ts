...
const CLASS_OVERRIDES: Record<string, ToolClass> = {
  threatlocker_computers_maintenance_modes: "read", // lists maintenance modes; does not start or end one
};
...
export function effectClassFor(name: string, description?: string): ToolClass {
  ...
  console.error(
    `[threatlocker-mcp] tool ${name} matches no effect-class pattern; annotating it as ` +
  ...
  return tools.map((t) => {
    if (t.annotations?.readOnlyHint !== undefined) {
      ...
      if (t.annotations.readOnlyHint === true && MUTATING_DESCRIPTION_MARKER.test(t.description ?? "")) {
        console.error(
          `[threatlocker-mcp] tool ${t.name} is annotated readOnlyHint:true but its own ` +
