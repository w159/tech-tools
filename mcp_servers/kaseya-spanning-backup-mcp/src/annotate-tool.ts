...
export function effectClassFor(name: string, description?: string): ToolClass {
  ...
  console.error(
    `[kaseya-spanning-backup-mcp] tool ${name} matches no effect-class pattern; annotating it as ` +
  ...
  return tools.map((t) => {
    if (t.annotations?.readOnlyHint !== undefined) {
      ...
      if (t.annotations.readOnlyHint === true && MUTATING_DESCRIPTION_MARKER.test(t.description ?? "")) {
        console.error(
          `[kaseya-spanning-backup-mcp] tool ${t.name} is annotated readOnlyHint:true but its own ` +
