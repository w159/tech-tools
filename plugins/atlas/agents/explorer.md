...
- **Optional prioritization (rank-then-verify).** When discovery surfaces more candidate files/patterns than you can read, use the rank-then-verify pattern in `${CLAUDE_PLUGIN_ROOT}/references/jev-patterns.md` to pick a reading order instead of reading all of them. One `typesafe_decide` call: a `choice` over every candidate with its one-line signature, plus a `noul` asking whether any candidate actually owns the behavior in question. Batch both into the same call - never one call per candidate. Read the winner **first**; it is a reading order, never a substitute for reading. If the abstain noul says none of them fit, say so rather than naming one anyway. Skip entirely and say nothing if the typesafe tools are unavailable.
