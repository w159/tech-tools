...
- When the claim under verification concerns code quality (type-safety, duplication, simplicity, frailty) rather than only pass/fail, optionally corroborate with `typesafe_decide` per `${CLAUDE_PLUGIN_ROOT}/references/jev-decisions.md`'s standard question set on the diff, all four questions in one call. Reduce it with `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/jev_reduce.py" --standard` rather than reading raw scores: scores are indices into their own rubrics, and `duplication` is a noul with no `confidence` to threshold. An answer whose band is `hold` is Jev declining to commit - treat it as no signal, not as agreement. A disagreeing Jev score is evidence to weigh alongside your own reproduction, never an automatic override of your verdict, and never grounds for `verified` on its own. Skip entirely and say nothing if the typesafe tools are unavailable.
