...
- When the claim under verification concerns code quality (type-safety, duplication, simplicity, frailty) rather than only pass/fail, optionally corroborate with `typesafe_decide` per `${CLAUDE_PLUGIN_ROOT}/references/jev-decisions.md`'s standard question set on the diff — a disagreeing Jev score is evidence to weigh alongside your own reproduction, never an automatic override of your verdict. Skip entirely and say nothing if the typesafe tools are unavailable.
