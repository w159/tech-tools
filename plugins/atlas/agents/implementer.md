...
**First serena call:** `activate_project` on the project cwd (or codebase root). Without it, every symbol tool fails.
...
- **Optional Jev signal.** After the change is complete and the diff is stable, optionally call `typesafe_decide` per `${CLAUDE_PLUGIN_ROOT}/references/jev-decisions.md`'s standard question set on the diff for a type-safety/duplication/simplicity/frailty signal. Below-threshold results go into the final report's "Anything you deliberately left out of scope" as a note, never a silent auto-fix. Skip entirely and say nothing if the typesafe tools are unavailable.
