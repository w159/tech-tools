...
0. **Orient (you, cheap).** Detect project + codebase roots (dirs with their own manifest). Read the *actual* run/test/build/lint commands from those manifests - never invent them. **Tool routing first (min context):** one batched ToolSearch per `references/tool-routing.md` / `scripts/tool_routing.py`; if code is present, **serena `activate_project` on cwd** before any symbol work; tree orient via lean-ctx `ctx_compose`/`ctx_search`, never Bash grep. **RECALL is a precondition to planning, not an optional nicety:** before you decompose any substantive task you MUST query `claude-mem` (`search` -> `timeline` -> `get_observations`, ids as numbers) and `ctx_search` - and the committed `.agents/` notes if present - for prior work, decisions, and gotchas on *this task + this project*, and your orientation MUST either cite what you found (source + the lesson you are reusing) or state explicitly "no prior work found". Then record the recall outcome once so run health reflects it: `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/atlas_db.py" record-recall "${CLAUDE_CODE_SESSION_ID}" hit` if recall surfaced a usable lesson, else `... miss` (fail-open: skip silently if the command is unavailable). Skipping recall and re-deriving a gotcha that memory already held is a violation of this step. Note live `serena`/lean-ctx/LSP/MCP/skill capabilities (and fallow on JS/TS). Scaffold per-root `docs/` artifacts. On boot or resume, reconcile recent work against docs/ per `${CLAUDE_SKILL_DIR}/references/session-lifecycle.md` before planning new work. Present a 5-10 line orientation + plan. **Gate before mutating anything.**
1. **Plan (you, `sequentialthinking` for non-trivial; `atlas:planner` for real decomposition).** Produce a **numbered stage map**: each stage yields exactly one verifiable artifact and names a **failable check** (the exact condition that would make the stage fail). The map is a **living document** in `docs/plans/` - update it as reality diverges. **Mirror it into `TodoWrite` immediately** (one todo per stage, see "The visible plan") - the stage map is for you, the todo list is for the user. Per stage, also fix: agent type, model tier, mandatory tools/skills. When a stage decomposes into multiple implementation units, give each a stable ID (`S<n>-U<m>`) in the stage map and pre-decide its evidence strategy (`proof-first` / `characterization` / `no-test-exception` - see `references/implementation-units.md`). Ambiguous *feature* work routes through `brainstorming` -> `make-plan` first. See `references/multi-stage-planning.md`.
2. **Dispatch (subagents, parallel).** Tight spec from `references/subagent-kit.md`, including the output contract from Token discipline. Each self-discovers best-fit capabilities, pulls Context7 docs for any library it touches, **executes to validate**, returns a short grounded report.
   ...
   **Implementation-unit discipline applies to every `atlas:implementer` dispatch (full checklists: `references/implementation-units.md`).** Four rules tighten what "tight spec" means when the target is a code-writing subagent:

   - **Idempotency first.** Before dispatching a unit - always when resuming an interrupted run - check whether its acceptance criterion already holds on the current tree. Already satisfied and matching intent: record the check, mark the unit complete, dispatch nothing. Partially satisfied: dispatch only the remaining slice and say which part already holds. Out-of-repo criteria (DNS records, console settings, live rows) decide from observed deliverable state, never from a clean tree.
   - **Name the evidence strategy in the prompt.** Per unit, state `proof-first` (write/strengthen the test, observe the expected red failure for the right reason, THEN implement), `characterization` (capture current legacy behavior in a passing baseline test before refactoring it), or `no-test-exception` (rename/pure-config/manual surface - with the named replacement verification). Never leave the choice implicit: the implementer is the only witness of red-before-green, and a choice it invents cannot be audited later. Include test discovery (existing test homes for every file the unit touches; reuse before duplicating) in the same prompt.
   - **Independent units run as bounded worktree waves.** Multiple `atlas:implementer` calls for genuinely independent units dispatch together in one message - after the Parallel Safety Check (committed dependencies; write sets disjoint AND no shared contracts/migrations/lockfiles/generated surfaces; no shared runtime singleton; acceptable merge cost; uncertainty resolved by reading the files, and contending units alone serialized) - isolated via the `atlas-worktree` skill, each worker given its base commit SHA to verify before editing, capped at 3-5 in flight, integrated in dependency order with actual-tree inspection, never on the strength of the reports alone.
   - **Demand the evidence receipt.** Each implementer's DELIVERABLE includes a `.atlas/evidence/<date>-<unit-id>-receipt.md` (changed files; evidence strategy; behavior_changed; verification command + observed result; when behavior changed - tests inspected/added/changed and the red or characterization observation) and the same fields condensed in REPORT BACK (step 3 folds them into findings.json).
3. **Verify (separate subagents) - the failable gate.** Run each stage's named failable check in a fresh context per law 5: reproduce the original failing case, derive an independent check, observe the result. This step asks "does the check pass on real execution?" - not "do I believe it works." **Write a `.atlas/.run/findings.json` entry for every implementer→verifier pair** so coverage is measurable and the completion gate's condition (g) has data to enforce. The format:
   ...
   **Fold each unit's evidence receipt into the ledger.** When a stage carried implementation units (step 2's discipline), stamp one findings entry per unit via the existing CLI - `--id <stage>-U<n>`, strategy and `behavior_changed` in `--title` ("retry path: proof-first, behavior_changed=yes"), changed files + test id + receipt path in repeatable `--evidence`, the verification command + actual result in `--reproduction` - so per-unit coverage is countable by the same gate that counts stages. The receipt is the implementer's self-report, not proof: a unit receipt never substitutes for the stage's independent check, and a stage with receipts but no check is still `pending`. A report missing receipt fields is not reconstructable from the tree afterward - recover what the tree allows, mark the rest unverified, and never fabricate the red-before-green observation the worker never reported. Full schema and example: `references/implementation-units.md`.
